...
function createIndexer(config, { logger, database, services, getSchema }) {
	...
	async function updateItemIndex(collection, ids) {
		...
		const pk = schema.collections[collection].primary;
		// console.warn("config.collections[collection].fields >> ", config.collections[collection].fields);
		...
		for (const item of items) {
			...
			try {
				...
				// console.warn("config.collections[collection].settings >> ", config.collections[collection].settings);
		...
		await indexer.updateSettings(collectionIndex, config.collections[collection].settings ? config.collections[collection].settings : {});
